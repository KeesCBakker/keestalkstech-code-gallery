...
public partial class InvalidJsonException(Exception ex, string jsonText) : Exception(ParseMessage(ex, jsonText), ex)
{
  ...
  public static string ParseMessage(Exception ex, string json, int linesAbove = 3, int linesUnder = 2)
  {
    ...
    if (match.Success)
    {
      ...
      var position = int.Parse(match.Groups["position"].Value, CultureInfo.InvariantCulture);

      var padding = (line + linesUnder).ToString(CultureInfo.InvariantCulture).Length;
      var lines = json.Split("\n")
          .Select((str, index) => $"{(index + 1).ToString(CultureInfo.InvariantCulture).PadLeft(padding, '0')} | {str}")
          .ToList();
